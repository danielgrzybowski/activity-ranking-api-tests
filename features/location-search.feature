...
Feature: Finding the city or town to plan around
  ...
  Background:
    ...
    And Open-Meteo's place catalogue contains:
      ...
      | 6299351  | London Heliport | England | United Kingdom | GB          | 51.4700  | -0.1810   | Europe/London       |            | AIRH        |
      | 2643743  | London   | England     | United Kingdom | GB          | 51.50853 | -0.12574  | Europe/London       | 8961989    |             |
      | 982299   | London   | Mpumalanga  | South Africa   | ZA          | -25.9992 | 30.9992   | Africa/Johannesburg |            |             |
      ...
      | 2265552  | Nazaré   |             | Portugal       | PT          | 39.60117 | -9.07048  | Europe/Lisbon       | 15158      |             |
  ...
  Scenario: A town with no region gets a display name without an empty gap
    It has no region for plenty more, and "Nazaré, , Portugal" in a dropdown is
    a bug the user sees. The label is built from the parts that exist.

    When I search for locations matching "Naza"
    Then the search results include "Nazaré, Portugal"
    And the result "Nazaré, Portugal" has no region
